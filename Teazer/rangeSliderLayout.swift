//
//  rangeSliderLayout.swift
//  Teazer
//
//  Created by Mraj singh on 22/11/17.
...
//    let videoMainViewController = VideoMainViewController()
    ...
    @IBInspectable var upperValue: Double = 60 {
        didSet {
    ...
    var gapBetweenThumbs: Double {
        print(" gap between thumbs :\(0.6 * Double(thumbWidth) * (maximumValue - minimumValue) / Double(bounds.width))")
    ...
    fileprivate var betweenThumbLayer = false
    ...
    override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        let doubleLocation = Double(previouslocation.x)
        print(doubleLocation/2.5)
        // Hit test the thumb layers
        if lowerThumbLayer.frame.contains(previouslocation) {
            lowerThumbLayer.highlighted = true
            lowerLayerSelected = lowerThumbLayer.highlighted
            betweenThumbLayer = false
        ...
        }else if (doubleLocation/2.5) < upperValue && (doubleLocation/2.5) > lowerValue {
            betweenThumbLayer = true
        }
        return lowerThumbLayer.highlighted || upperThumbLayer.highlighted || betweenThumbLayer
    }
    
    override func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        let location = touch.location(in: self)
        let doubleLocation = Double(previouslocation.x)
        print("countinue: \(doubleLocation/2.5)")
        ...
        if lowerThumbLayer.highlighted {
            ...
            if upperValue >= maximumValue{
                ...
                if upperValue - lowerValue < 60.0{
                    upperValue = maximumValue
                    lowerValue = boundValue(lowerValue + deltaValue, toLowerValue: minimumValue, upperValue: upperValue - gapBetweenThumbs)
                }else{
                    lowerValue = boundValue(lowerValue + deltaValue, toLowerValue: minimumValue, upperValue: upperValue - gapBetweenThumbs)
                    upperValue = lowerValue + 60.0
                }
            }else if upperValue - lowerValue > 60.0{
                lowerValue = boundValue(lowerValue + deltaValue, toLowerValue: minimumValue, upperValue: upperValue - gapBetweenThumbs)
                upperValue = lowerValue + 60.0
            }else{
                lowerValue = boundValue(lowerValue + deltaValue, toLowerValue: minimumValue, upperValue: upperValue - gapBetweenThumbs)
        ...
        }else if betweenThumbLayer && (doubleLocation/2.5) < upperValue && (doubleLocation/2.5) > lowerValue  {
